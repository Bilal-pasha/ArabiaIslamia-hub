...
const COLOR_ERROR = '#c53030';

export function renderAdmissionStatusEmail(options: AdmissionStatusTemplateOptions): string {
  const { applicantName, applicationNumber, status, reason, logoUrl, brandName = DEFAULT_BRAND } = options;
...
<head>
  ...
  <title>Application ${statusLabel}</title>
</head>
