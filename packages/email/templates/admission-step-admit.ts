...
export function renderAdmissionStepAdmitEmail(options: AdmissionStepAdmitTemplateOptions): string {
  const { applicantName, applicationNumber, stepName, stepDescription, admitCardUrl, logoUrl, brandName = DEFAULT_BRAND } = options;
  ...
  const admitBlock = admitCardUrl
    ? `
              ...
              <p style="margin:0 0 24px;text-align:center;">
                <a href="${escapeHtml(admitCardUrl)}" style="display:inline-block;background:${COLOR_ACCENT};color:${COLOR_WHITE};padding:14px 28px;text-decoration:none;font-weight:600;border-radius:8px;font-size:16px;">Download admit card</a>
              </p>`
    : '';
...
<head>
  ...
  <title>${escapeHtml(stepName)}</title>
</head>
